			...
			menufocus: function( event, ui ) {
				...
				if ( false !== this._trigger( "focus", event, { item: item } ) ) {
					...
					$.each(itemlist, function(i) {
						...
						if(txt1.length > 0) {
							if(txt === item[0]) {
								txt1.attr('src','//newsearch.seoul.go.kr/ksearch/assets/images/main/auto_arr2.gif');  // >>
								$(this).parents('li').css('background-color','#f5f5f5');
							}else {
								txt1.attr('src','//newsearch.seoul.go.kr/ksearch/assets/images/main/auto_arr1.gif');  // >
...
//		this.menu_wrap.children('.column-right').outerHeight(40);
	...
	_renderItem: function( ul, item ) {
		...
		.append($( (item.length > 1) ? "<img src=\"//newsearch.seoul.go.kr/ksearch/assets/images/main/auto_arr1.gif\" style=\"margin-left:-5px;border:0px;\">" : "") 	))
		.appendTo( ul );
	},
...
$.widget( "konan.rankings", {
	...
	refresh: function() {
		...
		if(window.XDomainRequest) {
			...
			if(xdr) {
				xdr3.onprogress = function() {};
				xdr3.open("get", this.options.url+"?domain_no="+this.options.domain_no+"&max_count="+this.options.max_count);
				xdr3.contentType = "text/json";
				xdr3.onload = function() { 
					...
					self._renderItems(JSON); 
				};
				setTimeout(function() { xdr3.send(); }, 0);
			}
		}else {
			$.ajax({
	...
	_renderItems: function(items) {
		var ol = $("<ul>").attr('data-trcode','pword').addClass("konan-rankings lst-keyword").appendTo(this.element.empty());
		var self = this;
		$.each(items, function(index, item) {
	...
	_getClassName: function(tag) {
		if (tag === "new") return "new"; else if (tag === "0") return "equal"; else if (tag.charAt(0) === '-') return "ico down"; else return "ico up";
	},
	
	_getClassText: function(tag) {
		if (tag === "new") return ""; else if (tag === "0") return "변동없음"; else if (tag.charAt(0) === '-') return "하락"; else return "상승";
	},
	
	_renderItem: function(ol, idx, item) {
		var style = idx < 3 ? "active" : "";
		var li = $( "<li class='"+style+"'>" ).append( "<span class='number'>"+(idx+1)+"</span>" ).append( $( "<a>" ).attr('href','javascript:void(0);').text(item[0]) );
		if (item.length > 1) {
			li.append( "<span class=\"right-side\"><span class='"+this._getClassName(item[1])+"'>"+(item[1]=="new" ? "NEW" : (item[1]=="0" ? "-" : ""))+"</span><span class=\"blind\">"+this._getClassText(item[1])+"</span></span>" );
		}
